...
def task(p, return_all=False):
    ...
    if sampler.keys:
        ...
        for sec, key in sampler.keys:
            if type(block[sec, key])==float:
                data_vectors.append(np.array([block[sec, key]]))
            else:
    ...
    if return_all:
        ...
        return r.like, data_vectors, r.prior, r.post
...
class EmulatorModule(ClassModule):
    ...
    def set_emulator_info(self, info):
        ...
        self.cosmopower = info["cosmopower"]
        self.emulator = None
    ...
    def execute(self, block):
        #print('in the emulator-execute function!!!')
        if self.cosmopower:
            ...
            # These need to be normalized as that's what the emulator is trained on
            ...
            # get prediction of data vector from emulator
        ...
        for (sec, key), val in self.fixed_inputs.items():
            block[sec, key] = val
        #print("output from emulator module: ", x.shape)    
        return 0
...
class EmugenSampler(ParallelSampler):
    ...
    def config(self):
        ...
        # modes of training: 
        # * gp = gaussian processing
        # * nn = custom neural networks of different architecture
        # * cp = cosmopower
        self.mode = self.read_ini_choices("mode", str, ["gp", "nn", "cp"])
        ...
        if save_outputs:
            ...
            if save_outputs=="model":
                ...
                # * 
                # *
                # * diagnostic plots: loss-function vs epoch, best-fit model in each itteration vs data
        ...
        print('SAVE_OUTPUT = ', self.save_outputs)    
        ...
        # batch sizes in each itteration
        self.batch_size = self.read_ini("batch_size", int, 32)
        # number of itterations per cycle
        ...
        print('TEMPERING: ', self.tempering)    
        # additional options (fixed by now)
        # data trafo
        self.fid_dv_flag = False
        # nn architecture
        self.nn_model = 'boruah'
        # COSMOPOWER OPTION
        if self.mode == 'cp':
            self.cosmopower = True
        else:
            self.cosmopower = False
    ...
    def generate_initial_sample(self):
        ...
        #print('Parameters normalised within their priors')
        #print('unit_sample: ', unit_sample)
        sample = np.array(
        ...
        print('sample_data_vectors = ', sample_data_vectors.shape)
        #print(sample_data_vectors[0])
        #print('sample chi2 = ', -2 * sample_likes)
    ...
    def train_emulator(self):
        ...
        if self.mode == "nn":
            #kwargs = {"n_epochs": self.training_iterations if self.iterations<self.max_iterations-1 else self.training_iterations*4, "batch_size": self.batch_size*(self.iterations+1)}
            kwargs = {"n_epochs": self.training_iterations*(self.iterations+4), "batch_size": self.batch_size*(self.iterations+1)}
            emu_class = NNEmulator
            #print("Architecture type: ", self.nn_model)
            # initialise emulator
            emu = emu_class(
                n_in, n_out, self.fiducial_data_vector, self.fiducial_errors,
                model_name=self.nn_model, optim=None, device='cpu', fid_dv_flag=self.fid_dv_flag
            )
            X = self.unit_sample
        elif self.mode == "cp":
            kwargs = {"model_filename": f'{self.save_outputs_dir}/emumodel_{self.iterations+1}', "n_epochs": self.training_iterations, "batch_size": self.batch_size*(self.iterations+1)}
            ...
            emu = emu_class(
                model_parameters, np.arange(n_out)
            )
            print('not noramlised self.sample: ', self.sample.shape)
    ...
    def load_emulator(self):
        ...
        if self.mode == "nn":
            ...
            with h5.File(filename + '.h5', 'r') as f:
                fiducial_data_vector = f['dv_fid'][:]
                fiducial_errors = f['dv_std'][:]
                n_in  = f['n_in']
                n_out  = f['n_out']
                model_name  = f['model_name']
            # initialise emulator
            emu = emu_class(
                n_in, n_out, fiducial_data_vector, fiducial_errors,
                model_name=model_name, optim=None, device='cpu'
            )
        elif self.mode == "cp":
            emu_class = CPEmulator
            model_parameters = [str(param) for param in self.pipeline.varied_params]
    ...
    def compute_fiducial_setup_emu_pipeline(self):
        ...
        #print('data_vectors: ',data_vectors)    
        #print('errors: ', errors)  
        ...
        for sec, key in block.keys(section="data_vector"):
            #print('key: ', key) #chi2, covariance, data, inv_cov, log_det, n, norm, sim, theory
        ...
        if len(emu_modules)!=0:
            ...
            #print(type(like_module))
            #print(dir(like_module))
            ...
            #raise NotImplementedError('Emulating on the data vector is not implemented yet')
        ...
        # why is it possible to set up the emulator module only after building the pipeline?
    ...
    def generate_updated_sample(self):
        ...
        # must guarantee to not overlap woth the training set
        random_index_test = []
    ...
    def execute(self):
        ...
        if  self.trained_before:
            ...
            self.train_emulator()
            ## save emulated models
            #if self.mode != 'cp':
            #    if self.save_outputs == SAVE_ALL or (self.save_outputs == SAVE_MODEL and self.iterations == self.max_iterations):
            #        self.emulator.save(f'{self.save_outputs_dir}/emumodel_{self.iterations+1}')
