...
class CobayaSampler(ParallelSampler):
    ...
    def execute(self):
        ...
        info = make_cobaya_info(fast_params, slow_params)
        print("Info:", info)
        ...
        for i in range(sample.n()):
            row = sample[i]
            out_row = [row[pname] for pname in pnames]
...
def make_main_likelihood(params):
    function_template = """
def likelihood({}):
    global pipeline
    ...
    function_text = function_template.format(params_text, length_text, set_text)
    exec(function_text)
    print(function_text)
    return locals()['likelihood']


def make_cobaya_info(fast_params, slow_params):
    # fast_params: list of param objects
    # slow_params: list of param objects
    likelihood = make_main_likelihood(slow_params + fast_params)
    ...
    for p in slow_params + fast_params:
        ...
        if isinstance(p.prior, priors.UniformPrior):
            pdict[pname] = {
                ...
                "proposal": 0.1*(p.prior.b-p.prior.a),
            }
