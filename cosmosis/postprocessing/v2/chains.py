...
class Chain:
    chain_subclasses = {}
    def __init__(self, name: str, sampler: str, columns: list[str], data: list[np.ndarray], metadata: list[dict[str,str]], comments: list[list[str]], final_metadata: list[list[str]], **options):
    ...
    def load(cls, inputs, name=None, **options):
        if isinstance(inputs, Inifile):
            name_, sampler, colnames, data, metadata, comments, final_metadata  = cls.load_ini(inputs)
        elif isinstance(inputs, str):
            name_, sampler, colnames, data, metadata, comments, final_metadata = cls.load_text_file(inputs)
        elif isinstance(inputs, output_module.InMemoryOutput):
            name_, sampler, colnames, data, metadata, comments, final_metadata = cls.load_in_memory_storage(inputs)
        elif isinstance(inputs, output_module.AstropyOutput):
            name_, sampler, colnames, data, metadata, comments, final_metadata = cls.load_astropy(inputs)
        elif isinstance(inputs, output_module.OutputBase):
            name_, sampler, colnames, data, metadata, comments, final_metadata = cls.load_output_object(inputs)
        else:
        ...
        return subclass(name, sampler, colnames, data, metadata, comments, final_metadata, **options)
    ...
    def load_ini(cls, inputs):
        ...
        colnames, data, metadata, comments, final_metadata = output_module.input_from_options(output_options)
    ...
    def load_text_file(cls, inputs):
        colnames, data, metadata, comments, final_metadata = output_module.TextColumnOutput.load_from_options({"filename":inputs})
        name = metadata[0].get("chain_name", "chain")
        sampler = metadata[0].get("sampler")
        return name, sampler, colnames, data, metadata, comments, final_metadata
    ...
    def load_output_object(self, inputs):
        colnames, data, metadata, comments, final_metadata = inputs.load_from_options({"filename":inputs})
        name = metadata[0].get("chain_name", "chain")
        sampler = metadata[0].get("sampler")
        return name, sampler, colnames, data, metadata, comments, final_metadata
    ...
    def mcsamples(self):
        ...
        if self._mcsamples is None:
            ...
            self._mcsamples = getdist.MCSamples(samples=samples, weights=weights, names=names)
