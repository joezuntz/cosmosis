...
class ChainCovariance(object):
    def run(self):
        ...
        col_names = [p for p in self.source.colnames if p.lower() not in ["post", "importance", "weight"]]
...
class GridStatistics(ConstrainingStatistics):
    def set_data(self):
        ...
        self.grid_columns = [i for i in xrange(self.ncol) if (not self.source.colnames[i] in extra) and (self.source.colnames[i]!="post")]
        self.ndim = len(self.grid_columns)
        assert self.nrow == self.nsample**self.ndim
        self.shape = np.repeat(self.nsample, self.ndim)
        self.like = np.exp(self.source.get_col("post")).reshape(self.shape)
