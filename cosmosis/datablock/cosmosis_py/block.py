...
class DataBlock(object):
	...
	def python_to_c_complex(value):
            ...
            components of a Python tuple `value`, or a real scalar `value`
            and return the equivalent lib.c_complex (i.e. a type which can
            be passed to a C subroutine representing a complex number).

            In the case of the scalar input, this is taken as the real
            part of the complex number and the imaginary part will be
            zero.
	...
	def python_to_1d_c_array(value, numpy_type):
            ...
            The object will be a contiguous list—this may entail that a
            value array with strides be copied to a compressed version—of C
            type most appropriate to the representation of the Python
            `numpy_type`.
	...
	def get_int(self, section, name, default=None):
            ...
            was given, or else a specialized :class:`BlockError` (see errors.py) will
            be thrown.  The :class:`BlockError` may also be thrown if a variable is
            found, but is not of integer type.
	...
	def get_bool(self, section, name, default=None):
            ...
            The `name`ʼd parameter in the given `section` will be interpreted as
            a boolean and returned to the caller.  If such parameter is not
            found in the map, then the `default` will be returned if it was
            given, or else a specialized :class:`BlockError` (see errors.py) will be
            thrown.  The :class:`BlockError` may also be thrown if a variable is
            found, but is not of boolean type.
	...
	def get_double(self, section, name, default=None):
            ...
            The `name`ʼd parameter in the given `section` will be interpreted as
            a floating-point value and returned to the caller.  If such
            parameter is not found in the map, then the `default` will be
            returned if it was given, or else a specialized :class:`BlockError` (see
            errors.py) will be thrown.  The :class:`BlockError` may also be thrown if
            a variable is found, but is not of floating-point type.
	...
	def get_complex(self, section, name, default=None):
            ...
            The `name`ʼd parameter in the given `section` will be interpreted as
            a complex value and returned to the caller.  If such parameter
            is not found in the map, then the `default` will be returned if it
            was given, or else a specialized :class:`BlockError` (see errors.py) will
            be thrown.  The :class:`BlockError` may also be thrown if a variable is
            found, but is not of complex type.
	...
	def get_string(self, section, name, default=None):
            ...
            The `name`ʼd parameter in the given `section` will be interpreted as
            a string value and returned to the caller.  If such parameter
            is not found in the map, then the `default` will be returned if it
            was given, or else a specialized :class:`BlockError` (see errors.py) will
            be thrown.  The :class:`BlockError` may also be thrown if a variable is
            found, but is not of string type.
	...
	def get_int_array_1d(self, section, name):
            ...
            The `name`ʼd parameter in the given `section` will be understood as
            being of integer array type and returned to the caller as a
            NumPy array.  If such a parameter is not found in the map, then
            a specialized :class:`BlockError` (see errors.py) will be thrown.
	...
	def put_double_array_nd(self, section, name, value):
            ...
            The `value` must be an array of values which can be
            interpreted as floating-point numbers, otherwise a
            :class:`ValueError` will be raised.  If the parameter does not
            exist in the data set, a :class:`BlockError` will be raised.
            The array can be any shape.
	...
	def put_int_array_nd(self, section, name, value):
            ...
            The value must be an array of values which can be interpreted
            as integer numbers, otherwise a :class:`ValueError` will be
            raised.  If the parameter does not exist in the data set, a
            :class:`BlockError` will be raised.  The array can be any
            shape.
	...
	def replace_double_array_nd(self, section, name, value):
            ...
            The value must be an array of values which can be interpreted
            as floating-point numbers, otherwise a :class:`ValueError`
            will be raised.  If the parameter already exists in the data
            set, a :class:`BlockError` will be raised.  The new array can
            be any shape, independent of the shape of the original value
            in this data set.
	...
	def replace_int_array_nd(self, section, name, value):
            ...
            shape, independent of the shape of the original value in this
            data set.
	...
	def get_double_array_nd(self, section, name):
            ...
            Expect :class:`BlockError` or :class:`ValueError` to be raised
	...
	def put_int(self, section, name, value):
            ...
            A new parameter will be added to the current map, at
            (`section`, `name`), and will have the `value` interpreted as
            an integer type.  It is an error to try to add a parameter
            which is already there, and in this case a specialized
	...
	def put_bool(self, section, name, value):
            ...
            A new parameter will be added to the current map, at
            (`section`, `name`), and will have the `value` interpreted as
            a boolean type.  It is an error to try to add a parameter
            which is already there, and in this case a :class:`BlockError`
            will be raised.
	...
	def put_double(self, section, name, value):
            ...
            A new parameter will be added to the current map, at
            (`section`, `name`), and will have the `value` interpreted as
            a floating-point type.  It is an error to try to add a
            parameter which is already there, and in this case a
            :class:`BlockError` will be raised.
	...
	def put_complex(self, section, name, value):
            ...
            A new parameter will be added to the current map, at
            (`section`, `name`), and will have the `value` interpreted as
            a complex type.  It is an error to try to add a parameter
            which is already there, and in this case a :class:`BlockError`
            will be raised.
	...
	def put_string(self, section, name, value):
            ...
            A new parameter will be added to the current map, at
            (`section`, `name`), and will have the `value` interpreted as
            a string type.  It is an error to try to add a parameter which
            is already there, and in this case a :class:`BlockError` will
            be raised.
	...
	def put_int_array_1d(self, section, name, value):
            ...
            A parameter called `name` is added to `section`, and holds
            `value` interpreted as a simple array of integers.  If this
            interpretation cannot be made then a :class:`BlockError` will
            be raised.
	...
	def put_double_array_1d(self, section, name, value):
            ...
            A parameter called `name` is added to `section`, and holds
            `value` interpreted as a simple array of floating-point
            values.  If this interpretation cannot be made then a
            :class:`BlockError` will be raised.
	...
	def get(self, section, name):
            ...
            The type value returned from this method will reflect the type
            of value held in the underlying map implementation.  In
            circumstances where this either cannot be ascertained or
            cannot be converted simply to a native Python type, then
            either a :class:`BlockError` or :class:`ValueError` will be
            raised.
	...
	def put(self, section, name, value, **meta):
            ...
            entry at (`section`, `name`), in which case a
            :class:`BlockError` specialization will be raised.
	...
	def replace_int(self, section, name, value):
            ...
            `value`.  It is an error to attempt to replace a value which
            is not already in the map, and a :class:`BlockError` will be
            raised in this case.
	...
	def replace_int_array_1d(self, section, name, value):
            ...
            If this cannot be done then a :class:`BlockError`
            specialization will be raised.
	...
	def __setitem__(self, section_name, value):
            ...
            The section_name must be a tuple with the parameterʼs section
            and name as the first two items (else a :class:`ValueError` will be
	...
	def __contains__(self, section_name):
            ...
            The section and name must be specified as the first two items
            of a tuple, or else a :class:`ValueError` will be raised.
            Normally a boolean is returned to indicate prescence of said
            parameter.  Implicit use of this method is the recommended way
            to determine if a parameter is present in the data set.
	...
	def save_to_file(self, dirname, clobber=False):
            ...
            extension; the path to the file will be created in the file
            system if necessary (:class:`ValueError` will be raised if
            this cannot be accomplished), and “.tgz” will be appended to
            the file name.
	...
	def report_failures(self):
            ...
            corresponding to the verb, section and name, and data-type of
            the parameter.
	...
	def get_log_entry(self, i):
            ...
            The return is a tuple of four strings indicating the verb
            (i.e., logged action), section and name of the parameter, and
            the data type held by the parameter.
	...
	def log_access(self, log_type, section, name):
            ...
            The `log_type` describes the action performed on the parameter
            at (`section`, `name`).  It should be one of the strings
            displayed in *datablock_logging.cc*, viz: "READ-OK", "WRITE-OK",
            "READ-FAIL", "WRITE-FAIL", "READ-DEFAULT", "REPLACE-OK",
            "REPLACE-FAIL", "CLEAR", "DELETE", or "MODULE-START".
	...
	def put_grid(self, section, name_x, x, name_y, y, name_z, z):
            ...
            The grid is put into `section`, using keys `name_x`, `name_y`
            and `name_z` to locate the data.  The data comprise the array
            `x` holding a set of ‘labels’ for the x-axis, an array `y`
            holding labels for the y-axis, and then a two-dimensional
            array`z`, whose sizes must correspond with the `x`- and
            `y`-sizes, which holds the actual data inside the grid.

            If there are any problems, most notably with the sizes of the
            arrays not being compatible, then a :class:`ValueError` will
            be raised.
	...
	def get_grid(self, section, name_x, name_y, name_z):
            ...
            `section` which index data making up a grid; they must be the
            same set used in a call to :func:`replace_grid` or
            :func:`put_grid` used to establish the grid in the first place
            (except that the x- and y-axes are allowed to be transposed).

            The return is a triple of arrays: the first two elements hold
            the labels along the axes and the third element is a
            two-dimensional array holding the data deemed to be inside the
            grid itself.
